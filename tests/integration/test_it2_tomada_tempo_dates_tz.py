...
def test_dates_and_tz_parsing(monkeypatch):
    # Arrange
    programming_html = _read_fixture("programming_timezone_dst.html")
    ...
    assert dates.issubset({"2025-08-01", "2025-08-02", "2025-08-03"})
    for e in events:
        assert e.get("time") is None or isinstance(e.get("time"), str)
