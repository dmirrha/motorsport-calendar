...
def test_tomada_tempo_malformed_html(monkeypatch):
    # Arrange
    programming_html = _read_fixture("programming_malformed.html")
    src = TomadaTempoSource()
    ...
    # Assert
    assert isinstance(events, list)
    assert len(events) >= 1  # o parser deve ser resiliente e extrair pelo menos 1 evento
    for e in events:
        assert e.get("date") is not None
        assert isinstance(e.get("streaming_links"), list)
    # Deve existir pelo menos um evento com link de streaming válido
    assert any(e for e in events if e.get("streaming_links"))
