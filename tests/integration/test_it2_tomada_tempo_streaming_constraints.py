...
def test_streaming_links_overflow(monkeypatch):
    programming_html = _read_fixture("programming_streaming_overflow.html")
    ...
    assert len(events) >= 1
    # Pelo menos um evento com mais de 3 links de streaming
    assert any(len(e.get("streaming_links", [])) >= 3 for e in events)
